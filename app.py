...
def dashboard():
    ...
    history = BorrowRecord.query.filter_by(user_id=current_user.id).filter(BorrowRecord.return_date.isnot(None)).all()
    plan = get_active_plan(current_user)
    return render_template('dashboard.html', borrows=borrows, history=history, plan=plan, limit=PLAN_LIMITS.get(plan, 2))
...
@app.route('/books/<int:book_id>/purchase', methods=['GET'])
@login_required
...
def checkout_book(book_id):
    ...
    # محاكاة عملية دفع وهمية (Mock Payment Gateway)
    card_number = request.form.get('card_number', '')
    if len(card_number.replace(' ', '')) < 12:
        flash('بيانات الدفع غير صحيحة، حاول مرة أخرى')
        return redirect(url_for('purchase_page', book_id=book.id))

    book.copies_available -= 1
    record = BorrowRecord(user_id=current_user.id, book_id=book.id, amount_paid=book.price)
    db.session.add(record)
    db.session.commit()
    logger.info(f'Mock payment successful: {book.title} - {book.price} SDG - by {current_user.email}')
    flash(f'تمت عملية الشراء بنجاح! تم دفع {book.price} جنيه (عملية وهمية)')
    return redirect(url_for('dashboard'))
...
def api_get_books():
    ...
    return jsonify([{
        ...
        'category': b.category, 'price': b.price, 'copies_available': b.copies_available
    } for b in books])
